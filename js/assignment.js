...
function addToCart (item) {
    ...
    console.log("[addToCart]: myCart count: " + myCart[itemID])
...
function displayCart () {
    ...
    const displayItem = function (item) {
        ...
        const quantity = parseInt(myCart[item.id])
        // remember that you can use the backtick method here (explained in the handout)
        cart.innerHTML += `<p>${"item " + item.id + ", quantity: " + quantity + ", $" + price}</p>`
    ...
    console.log("myCart:", keys)
    Object.keys(myCart).map(id => {
